...
class Viatura(db.Model):
    ...
    equipe = db.relationship('Equipe', backref='viaturas')
    deleted = db.Column(db.Boolean, default=False)
#    controles = db.relationship('ControlePark', backref='viatura', lazy=True)

    def __repr__(self):
        return f"<Viatura {self.vtr}>"
...
class ControlePark(db.Model):
    ...
    viatura = db.relationship('Viatura', backref='viatura_controles')  # Use um nome de backref único
    equipe = db.relationship('Equipe', backref='controles')  # Relacionamento com equipe
...
class GoogleForms(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    formulario = db.Column(db.String(100), nullable=False)
    descricao = db.Column(db.String(255), nullable=False)
    url = db.Column(db.String(255), nullable=False)
    roles = db.Column(db.String(255), nullable=False)  # Armazena os papéis/perfis como uma string

    def __repr__(self):
        return f"<GoogleForms {self.formulario}>"
...
class Equipe(db.Model):
    ...
    def __repr__(self):
        return f"<Equipe {self.equipe} - Suporte: {self.tipo_suporte}>"


class HistoricoViaturaEquipe(db.Model):
    __tablename__ = 'historico_viatura_equipe'
    id = db.Column(db.Integer, primary_key=True)
    viatura_id = db.Column(db.Integer, db.ForeignKey('viatura.id'), nullable=False)
    ...
    viatura = db.relationship('Viatura', backref='historico_equipes')
    equipe = db.relationship('Equipe', backref='historico_viaturas')

    def __repr__(self):
        return f'<HistoricoViaturaEquipe viatura={self.viatura_id} equipe={self.equipe_id}>'
